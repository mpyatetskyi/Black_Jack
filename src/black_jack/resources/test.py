...
class Game:
    ...
    def check_for_blackjack(self, value):
        if value == 21:
            return True

    def show_blackjack_result(self, name, result):
        if result == True:
    ...
    def play(self):
        ...
        while playing:
            ...
            self.dealer_hand.display(hide=True)

            game_over = False

            while not game_over:

                player_blackjack = self.check_for_blackjack(self.player_hand.value)

                if player_blackjack:
                    game_over = True
                    self.show_blackjack_result(self.player_hand)
